...
\subsection{File-level Implementation Structure}
\begin{table}[H]
    \centering
    \small
    \caption{Implementation Responsibilities by File and Module}
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{7.5cm}|X|}
        \hline
        \textbf{Path} & \textbf{Implementation Responsibility} \\
        \hline

        \shortstack[l]{\texttt{frontend-react/src/}\\\texttt{App.jsx}} & \shortstack[l]{Routing, protected pages,\\and user profile layout.} \\
        \hline

        \shortstack[l]{\texttt{frontend-react/src/}\\\texttt{services/api.js}} & \shortstack[l]{API calls, JWT handling,\\file upload, and token refresh.} \\
        \hline

        \shortstack[l]{\texttt{frontend-react/src/components/}\\\texttt{SubmissionDetail.jsx}} & \shortstack[l]{Submission review UI, source analysis,\\and polling status updates.} \\
        \hline

        \shortstack[l]{\texttt{frontend-react/src/components/}\\\texttt{MetadataForensics.jsx}} & \shortstack[l]{Metadata display, clustering results,\\and forensic visualization.} \\
        \hline

        \shortstack[l]{\texttt{backend-django/apps/}\\\texttt{authentication/}} & \shortstack[l]{User registration, verification,\\login, and profile management.} \\
        \hline

        \shortstack[l]{\texttt{backend-django/apps/}\\\texttt{classroom/}} & \shortstack[l]{Classroom, assignment, submission,\\and join-request logic.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/ml\_adapters/}\\\texttt{ocr\_engine.py}} & \shortstack[l]{Document parsing and\\OCR text extraction.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/ml\_adapters/}\\\texttt{detector\_service.py}} & \shortstack[l]{Text chunking, model scoring,\\and AI detection orchestration.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/ml\_adapters/}\\\texttt{firecrawl\_service.py}} & \shortstack[l]{Web search, source retrieval,\\and similarity checking.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/ml\_adapters/}\\\texttt{plagiarism\_vector.py}} & \shortstack[l]{Similarity comparison and\\vector-based plagiarism analysis.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/}\\\texttt{document\_forensics.py}} & \shortstack[l]{Metadata inspection, anomaly detection,\\and forensic reporting.} \\
        \hline

        \shortstack[l]{\texttt{analysis\_engine/}\\\texttt{tests.py}} & \shortstack[l]{Validation for analysis,\\OCR, and forensic modules.} \\
        \hline
    \end{tabularx}
\end{table}
\FloatBarrier
...
The React 19 (Vite) frontend delivers a role-based interface with distinct Student and Teacher dashboards. \texttt{SubmissionDetail.jsx} renders chunk-level AI-probability bars, triggers source verification, and polls for background results. \texttt{MetadataForensics.jsx} renders similarity clusters, the pairwise similarity matrix, metadata fields, heatmaps, and scatter plots. JWT-based authentication with automatic token refresh is functioning across all protected routes.

\subsection{Backend Architecture}

The Django REST Framework backend is organized into three functioning apps -- 

\texttt{authentication}, \texttt{classroom}, and \texttt{analysis\_engine} -- with role-filtered querysets enforcing that students, teachers, and administrators access only permitted data. Database integrity rules are enforced and confirmed working: unique classroom invite codes, one join request per student per classroom, one submission per student per assignment, and rejection of submission create/update after the assignment deadline.
