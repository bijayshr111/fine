...
Mitchell et al. \cite{mitchell} introduced a zero-shot approach to machine-generated text detection based on probability curvature. Their work reinforces the need to present AI-text scores as probabilistic indicators rather than definitive proof.

Potthast et al. \cite{potthast} provided an evaluation framework for plagiarism detection, supporting the use of multiple comparison measures and evidence-based review in this project.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.9\textwidth,height=0.72\textheight,keepaspectratio]{Images/mlworkflow1.png}
\caption{Machine Learning Model Workflow}
\label{fig:mlworkflow}
\end{figure}
The workflow shown in Figure~\ref{fig:mlworkflow} is organized into the following blocks:

\noindent\textbf{A. Document Input and Upload.} The process begins when a user uploads a document through the application. The system accepts common academic submission formats, including PDF, DOCX, and TXT files, and records the uploaded file for subsequent analysis.

\noindent\textbf{B. Document Extraction.} The system first determines whether the document contains directly readable text. For digital PDF, DOCX, and TXT files, the embedded text is extracted using the appropriate document parser. This text is then passed to the common analysis pipeline.

\noindent\textbf{C. OCR and HTR Processing.} If the document is scanned, image-based, or handwritten, the OCR module converts its visual content into machine-readable text. OCR and handwritten-text recognition methods are applied according to the document type and image quality. The extracted text is normalized before further analysis.

\noindent\textbf{D. Text Preprocessing and Chunking.} The extracted text is cleaned and divided into smaller sections of approximately 500 words. Chunking reduces the processing load for large documents, supports efficient model inference, and allows the final report to identify the sections that require closer review.

\noindent\textbf{E. AI-Text Detection.} Each text chunk is transformed into numerical features using TF-IDF vectorization and passed to the trained Multinomial Naive Bayes classifier. The classifier produces an AI-generated probability for every chunk. These probabilities are combined to calculate the document-level result, while a predefined threshold is used to mark chunks as likely AI-generated or likely human-written.

\noindent\textbf{F. Conditional Plagiarism Verification.} When the AI-generated probability exceeds the configured threshold, the system initiates plagiarism verification. The Firecrawl API searches for potentially related online sources, and the retrieved content is compared with the submitted document using similarity measures. This stage identifies overlapping passages and helps distinguish original AI-generated writing from AI-generated content that may have been copied or closely paraphrased.

\noindent\textbf{G. Result Aggregation.} The system combines the extraction result, chunk-level AI probabilities, similarity scores, matched source information, and supporting evidence. If the AI probability remains below the threshold, the optional source-verification stage can be skipped to reduce processing time and external API usage.

\noindent\textbf{H. Originality Report.} Finally, the system generates an Originality Report containing the overall AI-generated probability, classifier confidence, plagiarism status, similarity percentage, matched source URLs, and chunk-wise evidence. The report supports teacher review by presenting analytical indicators rather than making an automatic final judgement.
...
\noindent\textbf{Access and Classroom Management}\par\medskip
...
\noindent\textbf{Originality Analysis and Forensics}\par\medskip
